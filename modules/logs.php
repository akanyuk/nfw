...
class logs extends base_module {
    ...
    public static function get_browser($user_agent = "") {
        ...
        if (!isset(NFW::i()->cfg['use_get_browser']) || !NFW::i()->cfg['use_get_browser'] || !$user_agent) {
            return "";
        }

        $b = get_browser($user_agent);
        if ($b === false || !isset($b->parent)) {
            return "";
        }
    ...
    public static function write($message, $kind = 0, $additional = false) {
        if (!isset(NFW::i()->cfg['write_logs']) || !NFW::i()->cfg['write_logs']) {
            return true;
        }
        ...
        if (!NFW::i()->db->query_build(array('INSERT' => implode(', ', $names), 'INTO' => 'logs', 'VALUES' => implode(', ', $values)))) {
            return false;
        }

...
/**
 * @desc Модуль работы с изображениями и файлами
 */
class media extends active_record {
    ...
    protected $images_cache_path = 'var/images_cache/';     // Path to images cache
    ...
    function __construct($record_id = false, $params = array()) {
        ...
        if (isset(NFW::i()->cfg['media']['storage_full_path'])) {
            ...
            $this->storage_path = PROJECT_ROOT . NFW::i()->cfg['media']['storage_path'];
        }

        if (isset(NFW::i()->cfg['media']['secure_storage_full_path'])) {
            $this->secure_storage_path = NFW::i()->cfg['media']['secure_storage_full_path'];
        } else if (isset(NFW::i()->cfg['media']['secure_storage_path']) && defined('PROJECT_ROOT')) {
            $this->secure_storage_path = PROJECT_ROOT . NFW::i()->cfg['media']['secure_storage_path'];
        }

        if (isset(NFW::i()->cfg['media']['images_cache_full_path'])) {
            $this->images_cache_path = NFW::i()->cfg['media']['images_cache_full_path'];
        } else if (isset(NFW::i()->cfg['media']['images_cache_path']) && defined('PROJECT_ROOT')) {
            $this->images_cache_path = PROJECT_ROOT . NFW::i()->cfg['media']['images_cache_path'];
        }

        if (isset(NFW::i()->cfg['media']['assets_full_path'])) {
            $this->assets_path = NFW::i()->cfg['media']['assets_full_path'];
        } else if (isset(NFW::i()->cfg['media']['assets_path']) && defined('PROJECT_ROOT')) {
            $this->assets_path = PROJECT_ROOT . NFW::i()->cfg['media']['assets_path'];
        }
    ...
    private function removeFile($record) {
        if ($record['secure_storage']) {
            @unlink($this->secure_storage_path . '/' . date('Y', $record['posted']) . '/' . $record['id']);
            @unlink($this->secure_storage_path . '/' . $record['id']);
        } else {
            @unlink($this->storage_path . '/' . $record['owner_class'] . '/' . $record['basename']);
        }

        // remove cached thumbnails
        $prefix = sprintf("%08s", $record['id']);
        if ($dh = opendir($this->images_cache_path)) {
            while (($file = readdir($dh)) !== false) {
                if (substr($file, 0, 8) == $prefix) {
                    @unlink($this->images_cache_path . '/' . $file);
                }
            }
    ...
    protected function formatRecord($record) {
        ...
        $record['url'] = $record['secure_storage'] ? NFW::i()->absolute_path . '/' . $this->media_controller . '/_protected/' . $record['owner_class'] . '/' . $record['id'] . '/' . $record['basename'] : NFW::i()->absolute_path . '/' . $this->storage_path . '/' . $record['owner_class'] . '/' . $record['basename'];

        if ($record['secure_storage']) {
            $record['fullpath'] = file_exists($this->secure_storage_path . '/' . date('Y', $record['posted']) . '/' . $record['id']) ? $this->secure_storage_path . '/' . date('Y', $record['posted']) . '/' . $record['id'] : $this->secure_storage_path . '/' . $record['id'];
            $record['tmb_dir'] = $this->images_cache_path . '/';
        } else {
            $record['fullpath'] = $this->storage_path . '/' . $record['owner_class'] . '/' . $record['basename'];
        }

        if ($record['type'] == 'image') {
            $record['tmb_prefix'] = $record['secure_storage'] ? NFW::i()->absolute_path . '/' . $this->media_controller . '/_protected/' . $record['owner_class'] . '/' . $record['id'] . '/_tmb' : NFW::i()->absolute_path . '/' . $this->storage_path . '/' . $record['owner_class'] . '/' . $record['filename'] . '_tmb';
        }
    ...
    protected function upload($file, $params = array()) {
        ...
        if (!$this->session['secure_storage'] && file_exists($this->storage_path . '/' . $this->session['owner_class'] . '/' . $this->record['basename'])) {
            if (isset($this->session['force_overwrite']) && $this->session['force_overwrite']) {
                @unlink($this->storage_path . '/' . $this->session['owner_class'] . '/' . $this->record['basename']);
            } else if (isset($this->session['force_rename']) && $this->session['force_rename']) {
                $target_dir = $this->storage_path . '/' . $this->session['owner_class'] . '/';
        ...
        if ($this->session['secure_storage']) {
            if (!file_exists($this->secure_storage_path . '/' . date('Y'))) {
                mkdir($this->secure_storage_path . '/' . date('Y'), 0777);
            }

            $target_file = $this->secure_storage_path . '/' . date('Y') . '/' . $this->record['id'];
        } else {
            $target_file = $this->storage_path . '/' . $this->session['owner_class'] . '/' . $this->record['basename'];
        }
    ...
    public function openSession($options, $form_data = array()) {
        ...
        $_data['secure_storage'] = isset($_data['secure_storage']) && $_data['secure_storage'];

        if (!$_data['secure_storage'] && !file_exists($this->storage_path . '/' . $_data['owner_class'])) {
            $this->error('Storage path not found: ' . $this->storage_path . '/' . $_data['owner_class'], __FILE__, __LINE__);
            return false;
        }
    ...
    function getOwnerFiles($owner_class = '') {
        if (!file_exists($this->storage_path . '/' . $owner_class)) {

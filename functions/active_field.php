...
function active_field($params, $additional = null) {
        ...
        <div data-active-container="<?php echo $id ? $id : $name ?>" class="form-group form-group-checkbox clearfix">
            ...
            <div class="checkbox">
                <label>
                    ...
                    <input <?php echo $input_id_name ?> type="checkbox"
    ...
    <div data-active-container="<?php echo $id ? $id : $name ?>" class="form-group clearfix">
        <?php if ($labelDesc): ?>
            <label for="<?php echo $id ? $id : $name ?>"
                   class="<?php echo $vertical ? '' : 'col-md-' . $labelCols . ' ' ?>control-label"><?php echo $required ? '<strong>' . $labelDesc . '</strong>' : $labelDesc ?></label>
        <?php endif; ?>
        ...
        <?php if ($type == 'date'):
            ...
            <div data-language="<?php echo NFW::i()->user['language'] ?>" style="white-space: nowrap;">
                <input type="text" name="<?php echo $name ?>"
                       value="<?php echo $value ? date($withTime ? 'd.m.Y H:i' : 'd.m.Y', $value) : '' ?>"
                       class="form-control" style="display: inline; width: 150px;"
                       data-datepicker="1"
                       data-editable="<?php echo $editable ? '1' : '0' ?>"
                       data-startdate="<?php echo date($withTime ? 'd.m.Y H:i' : 'd.m.Y', $startDate) ?>"
                       data-enddate="<?php echo date($withTime ? 'd.m.Y H:i' : 'd.m.Y', $endDate) ?>"
                       data-unixtimestamp="<?php echo intval($value) ?>"
                       data-withtime="<?php echo $withTime ?>"/><span id="set-date" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-calendar"></span></span><?php echo $required ? '' : '<span id="remove-date" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-remove"></span></span>' ?>
            </div>
        <?php elseif ($type == 'select'): ?>
            <select <?php echo $input_id_name ?>
                    class="form-control<?php echo $class ? ' ' . $class : '' ?>"><?php foreach ($options as $o) { ?>
                    <?php if (is_array($o)): ?>
        ...
        <?php elseif ($type == 'password'): ?>
            <input type="password" <?php echo $input_id_name ?>
                   class="form-control <?php echo $class ?>" <?php echo $placeholder . $maxlength ?>
                   value="<?php echo htmlspecialchars($value) ?>"/>
        <?php elseif ($type == 'int' || $type == 'float'): ?>
            <input type="number" <?php echo $input_id_name . $min . $max . $style ?> step="<?php echo $step ?>"
                   class="form-control <?php echo $class ?>"<?php echo $placeholder ?>
                   value="<?php echo htmlspecialchars($value) ?>"/>
        <?php else: ?>
            <input type="text" <?php echo $input_id_name ?>
                   class="form-control <?php echo $class ?>"<?php echo $placeholder . $maxlength . $style ?>
                   value="<?php echo htmlspecialchars($value) ?>"/>
        <?php endif; ?>

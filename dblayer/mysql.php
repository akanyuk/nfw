...
if (!function_exists('mysql_connect'))
	exit('This PHP environment doesn\'t have MySQL support built in. MySQL support is required if you want to use a MySQL database to run. Consult the PHP documentation for further assistance.');
...
class DBLayer
{
	...
	function DBLayer($db_host, $db_username, $db_password, $db_name, $db_prefix, $p_connect)
	{
		...
		if ($this->link_id)
		{
			if (!@mysql_select_db($db_name, $this->link_id))
				NFW::i()->stop('Unable to select database.');
		}
		else
			NFW::i()->stop('Unable to connect to MySQL server.');

		// Setup the client-server character set (UTF-8)
		if (!defined('NFW_NO_SET_NAMES'))
			$this->set_names('utf8');
	...
	function query($sql, $unbuffered = false)
	{
		...
		{
			if (defined('NFW_LOG_QUERIES'))
				$this->saved_queries[] = array($sql, sprintf('%.5f', NFW::i()->microtime() - $q_start));
	...
	function create_table($table_name, $schema, $no_prefix = false)
	{
		...
		if (!$this->query($query)) {
			NFW::i()->stop('Unable to create table.');
		}
	}
